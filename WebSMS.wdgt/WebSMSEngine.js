...
WebSMSEngine.prototype.responseDidBecomeAvailable = function(response)
{
	...
	if (++this._step < this._plugin.steps.length)
	{
		...
		{
			if (typeof(this._plugin.steps[this._step].delay == 'number'))
			{
				var delay = this._plugin.steps[this._step].delay;
				__DEBUG("[WebSMSEngine] Delayed call %s msec".sprintf(delay));
